...
| 14 | PGDATA-001 | P2 | **讓非衝突、已審核的 public-goods cases 成為 QA 資料來源** | Schema 0.2、完整 Draft 2020-12／跨紀錄驗證、現有 seed migration、OSKey／OpenRPC 首批無已知金額衝突的 draft 匯入，以及不含 voter identity／wallet／會議密碼的 sanitized application evidence 已完成；剩餘：內容 owner review、定義 QA 可用欄位、runtime 不再只靠 `projects.yaml`、回應帶 provenance／日期、private/internal 及 `pending_reconciliation` claims 不進 prompt；review/scoring 接入另由 REVIEW-001 控制 | QA-FACT-001、PRIV-001A／PRIV-001H |
